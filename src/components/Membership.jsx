                    ...
                    {tiers.map((tier, index) => (
                        <TiltCard key={index} className="h-full group">
                            <motion.div
                                ...
                                className={`relative p-8 rounded-3xl bg-bg-secondary border ${tier.color} ${tier.glow} group-hover:-translate-y-2 transition-transform duration-300 flex flex-col h-full ${tier.popular ? 'scale-105 z-10 ring-1 ring-accent/50' : 'opacity-90 group-hover:opacity-100'}`}
                            >
                                {tier.popular && (
